...
public class PfaseJoin:PfaseSr
{
    ...
    public void ActButtonClick(Game _g)
    {
        ...
        {
            _g.IsShow.ShowMessage("Этот отряд не пренадлежит вам.");
        }
        else
        {
                    ...
                    foreach (BasicModel c_model in unit.Models)
                    {
                        if (c_model!=t_model)
                        {
                            if (Math.Sqrt((c_model.x - t_model.x) * (c_model.x - t_model.x) + (c_model.y - t_model.y) * (c_model.x - t_model.y))-100 <= _g.enemy_distance)
                            {
                                ...
                                {
                                    en = 1;
                                    MessageBox.Show("Вы не можете пройти через вражеские модели. " + _g.cur_model.x.ToString() + " " + _g.cur_model.y.ToString() + " " + x.ToString() + " " + y.ToString() + " " +
